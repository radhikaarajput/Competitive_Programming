...
public:
    vector<int> candyStore(int candies[], int N, int K)
    {
        ...
        //payForCandy--> no of candies for which we have to pay
        //{3 2 1 4}
        //N=4, k=2   ---- payForCandy=(4+2)/3--->2 
        
        //{3 2 1 4 5}  n=5,k=4
        //payForCandy=(5+4)/5--->1

        int payForCandy=(N+K)/(K+1);
        sort(candies, candies+N);
        
        for(int i=0;i<payForCandy;i++)
        {
            //minCost at begining
            minCost+=candies[i];
        }
        ...
        for(int i=N-1; i>=N-payForCandy; i--)
        {
            ////max candies at end
            maxCost+=candies[i];
        }
...
/*
Shop in Candy Store 
Easy Accuracy: 62.69% Submissions: 3413 Points: 2
